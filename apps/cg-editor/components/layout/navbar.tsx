...
export default function Navbar() {
   ...
   return (
      ...
      <div className="fixed top-3 left-0 right-0 flex justify-center w-full z-50">
         <div className="flex items-center px-1.5 py-1.5 bg-background rounded-full">
            {menuItems.map((item) => (
               <Link
                  key={item}
                  ...
                  className={cn(
                     "px-5 py-2 mx-0.5 text-[12px] font-medium text-primary transition-colors rounded-full",
                     activeItem === item
